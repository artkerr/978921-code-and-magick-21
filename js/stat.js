...
var getBlueColor = () => `hsla(231, 90%, ${Math.round(Math.random() * (80 - 20)) + 20}%, 1)`;

var getMaxElement = (arr) => Math.max.apply(null, arr);
...
var renderStatistics = (ctx, names, times) => {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillText(Math.round(times[i]), CLOUD_X + BAR_WIDTH + (BAR_WIDTH + BAR_GAP) * i, BAR_MAX_HEIGHT * times[i] / maxTime);
